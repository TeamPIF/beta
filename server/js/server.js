...
var pool = mysql.createPool({
    host : "nadamvp.cd8mpiv2feon.us-west-2.rds.amazonaws.com",
    port : 3306,
    user : "foodforchange",
    password : "Chelseaforchange8",
    database : "ffcMVP"
});
...
apiRouter.get(routes.apiGetRoutes.stats_donated, function(req, res){
    ...
    pool.getConnection(function(err, connection) {
        ...
        connection.query(query, function(err, rows) {
            if (err) throw err;
            res.json({ result : rows[0].donations });
        });
        connection.release();
    });
});
...
apiRouter.get(routes.apiGetRoutes.stats_partners, function(req, res){
    ...
    pool.getConnection(function(err, connection) {
        ...
        connection.query(query, function(err, rows) {
            if (err) throw err;
            res.json({ result : rows[0].partners });
        });
        connection.release();
    });
});
